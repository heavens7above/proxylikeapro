...
// HTTP Agent with keepAlive for connection pooling
const httpAgent = new http.Agent({
  keepAlive: true,
  maxSockets: 100,
});

// HTTPS Agent with keepAlive for connection pooling
...
const proxyMiddlewareHttps = createProxyMiddleware({
  ...commonProxyOptions,
  target: 'https://localhost', // fallback, overridden by router
  agent: httpsAgent,
});
...
const handleProxy = (req, res, next) => {
  ...
  // Dispatch based on protocol to use appropriate keepAlive agent
