...
## 2026-03-07 - Winston Log Formatting Optimization
**Learning:** `morgan` passes objects to Winston via `info.message`. Previously, `winston.format.printf` was destructured and attempted to parse JSON strings fallback, or re-assigned `httpLog`. Because `morgan` was already set up to return an object (though previous iterations of this codebase had it serialize JSON), Winston merges the object properties from `morgan` directly into the `info` object if not careful, or if `morgan` returns an object. Actually, in `app.js`, `morgan` was changed to pass the object directly to `logger.http(httpLog)` which avoids JSON serialization entirely. Winston correctly merges the object into `info`. We don't need to try to parse it from `info.message` if it's already destructured into `info` directly, which speeds up formatting.
**Action:** When `morgan` returns structured objects to `winston`, ensure the format function directly reads from `info` rather than attempting unnecessary `JSON.parse` or checking `info.message` unnecessarily.
