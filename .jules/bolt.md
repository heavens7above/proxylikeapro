...
## 2026-08-14 - [Static Configuration Logging]
**Learning:** Logging static configuration warnings (e.g., missing proxy password) on every request in high-throughput middleware creates significant console/file I/O bottlenecks.
**Action:** Use a module-level flag to ensure such warnings are logged only once instead of per-request. Also, prefer module-scoped variables (`let flag = false`) over polluting the global namespace (`global.flag`).
