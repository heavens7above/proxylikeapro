...
## 2026-06-13 - Suspend Background Polling
**Learning:** Polling via `setInterval` runs blindly even when the user's browser tab is not visible, consuming unnecessary network and CPU resources for both the client and server.
**Action:** Use `document.visibilityState === 'visible'` to conditionally execute background tasks.
